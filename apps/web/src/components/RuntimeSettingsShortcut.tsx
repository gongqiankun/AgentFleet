...
export function RuntimeSettingsShortcut({sessionId,summary,observed,running,onOpen}:{sessionId:string;summary?:RuntimeSummary;observed?:RuntimeSettings|null;running:boolean;onOpen:()=>void}) {
  ...
  const title=`${detail} · ${t("点击配置下次发送的模型与推理强度")}${!current?.settings&&native?` · ${t("模型信息来自最近一次主机记录")}`:""}`;
  return <button type="button" className="runtime-settings-shortcut" aria-label={t("快速配置模型与推理强度")} aria-haspopup="dialog" title={title} onClick={onOpen}><Settings2 size={13}/>{running&&<small>{t("下轮")}</small>}<span>{text}</span><ChevronRight size={13}/></button>;
}
