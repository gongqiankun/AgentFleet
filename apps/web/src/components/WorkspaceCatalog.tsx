...
function SessionRows({ sessions, selectedId, onSelect }: { sessions: FleetSession[]; selectedId?: string; onSelect: (id: string) => void }) {
  return <div className="session-list">{sessions.map((session) => <button type="button" key={session.id} className={`session-row${selectedId === session.id ? " session-row--active" : ""}`} onClick={() => onSelect(session.id)}>
    ...
    <div className="session-row__bottom"><span className="state-pill">{session.state.unknownFreeze ? t("结果待核验") : session.state.reachability !== "live" ? t("离线") : session.state.waitReason === "approval" ? t("等待确认") : session.state.waitReason === "user_input" ? t("等待回答") : session.state.currentTurn === "in_progress" ? t("正在执行") : t("空闲")}</span><span className="history-mark">{session.state.history === "complete" ? t("完整历史") : session.state.history === "partial" ? t("部分历史") : t("历史摘要")}</span><span className="session-row__tokens" title={session.recordedTokens == null ? t("未记录") : t("已记录 {0} tokens", new Intl.NumberFormat(locale()).format(session.recordedTokens))}>{session.recordedTokens == null ? t("未记录") : t("已记录 {0} tokens", new Intl.NumberFormat(locale(), { notation: "compact", maximumFractionDigits: 1 }).format(session.recordedTokens))}</span></div>
  </button>)}</div>;
}
