...
- Snapshots older than three minutes are marked out of date. API-key authentication or unsupported quota responses may provide no quota window. Missing data never means zero use or unlimited quota.
...
同账号额度不能跨主机相加，也不能按 token 比例分摊到项目。额度最多每分钟查询一次，页面每 30 秒刷新；超过三分钟未更新会标注过期。没有上报不是零消耗或无限额度。最近 7 个 UTC 日期与账号周额度重置周期是两个不同范围。
