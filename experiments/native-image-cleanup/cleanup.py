"""Linux, Codex 0.153.4 experiment; deliberately restricted to isolated fixtures.

Not imported by AgentFleet and not a production cleanup endpoint. Keep rollout
line byte lengths stable so native persisted byte offsets remain valid even if
execution stops between the atomic rollout replacement and SQLite commit.
"""
...
def fixture_home(home):
    ...
    if not home.is_absolute() or home.resolve() != home:
        raise CleanupError("Noncanonical fixture path")
    marker = home / '.agentfleet-image-cleanup-fixture'
    regular(marker)
    if marker.read_text() != 'isolated-no-account-no-user-data\n':
        raise CleanupError("Not an isolated fixture")
    if (home / 'auth.json').exists():
        raise CleanupError("Account credentials present; refuse fixture experiment")
    config = home / 'config.toml'
    regular(config)
    if 'model_provider = "fixture"' not in config.read_text():
        raise CleanupError("Fixture provider required")
    return home
...
def cleanup_fixture(home, rollout, thread_id, turn_id, image_hashes, expected_digest, *, failpoint=None):
    """Remove selected turn's inline images; repeat with current digest to reconcile.

    Private synthetic fixture only. Failpoints are for crash-window regression.
    Receipt means logical image content was removed, NOT forensic erasure or
    filesystem space reclamation. Other stores/Agent journals are out of scope.
    """
    home = fixture_home(home)
    rollout = Path(rollout)
    if rollout.resolve() != rollout or not rollout.is_relative_to(home / 'sessions'):
        raise CleanupError("Rollout outside fixture session directory")
    hashes = set(image_hashes)
    if not hashes or len(hashes) > 4 or any(len(h) != 64 or any(c not in '0123456789abcdef' for c in h) for h in hashes):
    ...
    with writer_lock(home, thread_id):
        ...
        output, changed = rollout_plan(data, thread_id, turn_id, hashes)
        ...
        try:
            ...
            if failpoint == 'before_replace':
                raise CleanupError('Injected interruption before replacement')
            if changed:
                ...
                try:
                    ...
                    os.close(directory)
            if failpoint == 'crash_after_replace':
                os._exit(73)  # Test real process death with an open SQLite transaction.
            if failpoint == 'after_replace':
                raise CleanupError('Injected interruption after replacement; projection may still contain image')
            db.executemany('UPDATE thread_items SET item_json=? WHERE thread_id=? AND turn_id=? AND item_id=?', updates)
            db.commit()
            return {'threadId': thread_id, 'turnId': turn_id, 'changedRolloutRecords': changed,
                    'changedProjectedItems': len(updates), 'beforeSha256': expected_digest,
                    'afterSha256': digest(output), 'rolloutBytes': len(output), 'byteOffsetsPreserved': True}
